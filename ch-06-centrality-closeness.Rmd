...
As discussed in the chapter on [degree centrality](), the measure of centrality is affected by the size of the graph, *g*. In the case of *closeness* centrality, having to sum over more nodes will push scores closer to zero. This means that we can't compare closeness centrality scoress across graphs of different sizes. *Solution*?
...
As discussed in [Chapter 5: Degree Centrality](https://jacobtnyoung.github.io/sna-textbook/ch-05-centrality-degree.html), when we have a directed graph, we have to consider directionality when evaluating a centrality measure. For example, in the graph below, *how far is Jen from Tom?* *How far is Jen from Bob?*

```{r, echo=FALSE, eval=TRUE, fig.width=5, fig.height=5, results="show", fig.align="center"}
